...
// Fetch terms of service content and SEO settings from database
$terms_content = $settings['terms_of_service_content'] ?? '';
$terms_last_updated = $settings['terms_last_updated'] ?? '';
$terms_meta_title = $settings['terms_meta_title'] ?? 'Terms of Service - ' . ($settings['company_name'] ?? '');
$terms_meta_description = $settings['terms_meta_description'] ?? 'Read our Terms of Service to understand the rules and guidelines for using our website and services.';

// If no custom content, use default
if (empty($terms_content)) {
    $terms_content = '<!-- Default terms of service content will go here -->';
}

// Set page variables for SEO
$page_title = htmlspecialchars($terms_meta_title);
$page_description = htmlspecialchars($terms_meta_description);
...
<head>
    ...
    <script type="application/ld+json">
    {
        ...
        "lastReviewed": "<?php echo !empty($terms_last_updated) ? date('Y-m-d', strtotime($terms_last_updated)) : date('Y-m-d'); ?>",
        "publisher": {
            "@type": "Organization",
...
<body>
                ...
                <div class="col-lg-10 mx-auto">
                    ...
                    <div class="last-updated">
                        <p class="mb-0">
                            ...
                            <?php if (!empty($terms_last_updated)): ?>
                                <?php echo date('F j, Y', strtotime($terms_last_updated)); ?>
                            <?php else: ?>
                                <?php echo date('F j, Y'); ?>
                            <?php endif; ?>
                        </p>
                    ...
                    <div class="related-links mt-4">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <a href="privacy-policy.php" class="btn btn-outline-primary w-100 mb-2">
    ...
    <script>
        // Add smooth scrolling for anchor links within the terms of service
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (lastUpdated) {
                const updateDate = new Date('<?php echo !empty($terms_last_updated) ? $terms_last_updated : date('Y-m-d H:i:s'); ?>');
                const now = new Date();
